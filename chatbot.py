...
intents = [
    ...
    },
    {
        ...
        "responses": [
            "Did you hear about the mathematician who is affraid of negative numbers? He will stop at nothing to avoid them!",
...
# Training the model
vectorizer = TfidfVectorizer(tokenizer=lambda x: re.findall(r'\w+', x.lower()), stop_words='english')
classifier = LogisticRegression(random_state=0, max_iter=5000, solver='liblinear', C=10)
...
st.title("A to Z 🤖 Interactive Chatbot")
st.write('''Chat with your AI assistant below : \n
         
         Instructions :
         Please just enter a keyword like (Artificial Intelligence, Coding, Fitness, 
         Robotics, Joke, Books, Age, Weather, Budget, Name, Gardening,nso on etc....)\n
         Question format : 1. Tell me a joke? \n
                           2. What's the best programming language?
                           ''')
...
for chat in st.session_state["chat_history"]:
    bubble_class = "chat-bubble-user" if chat["sender"] == "user" else "chat-bubble-bot"
    st.markdown(f'<div class="{bubble_class}">{chat["message"]}</div>', unsafe_allow_html=True)
